...
from environment.models import Action, Observation, Reward, DroneAction, HOVER
...
# Landing page removed to allow Gradio to own the root route
...
with gr.Blocks(title="Drone Traffic Control — OpenEnv", css=".gradio-container { max-width: 1200px !important; }") as demo:
        ...
        with gr.Tab("🚀 Simulation Hub", id=0):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    task_info_box = gr.Markdown(TASK_INFO["hard"])
        ...
        with gr.Tab("📖 System Documentation & Gallery", id=1):
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    4. **Physics Mode**: Injects wind variability and drone dynamics into the environment.
                    """)
            ...
            gallery = gr.Gallery(
                value=gallery_paths,
                label="AirSim Autonomous Navigation (6 Key Frames)",
                show_label=True,
                elem_id="gallery",
                columns=[3],
                rows=[2],
    ...
    # Navigation Logic
    doc_btn.click(fn=lambda: gr.update(selected=1), outputs=tabs)
    back_btn.click(fn=lambda: gr.update(selected=0), outputs=tabs)
    gallery_jump_link.click(fn=lambda: gr.update(selected=1), outputs=tabs)
...
# Mount Gradio app into FastAPI at the root route
